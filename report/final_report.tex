...
\begin{itemize}
    \item \textbf{Crazy compression ratios.} Similar data types pack together efficiently. ClickHouse uses LZ4 (default) or ZSTD compression on columnar data.
    \item \textbf{Blazing fast aggregations.} Reading one column from disk is way faster than reading entire rows.
\end{itemize}

The MergeTree storage engine handles this, automatically merging data in the background and keeping everything indexed by your primary key \cite{clickhouse_docs}. Data is stored in parts, and background threads continuously merge smaller parts into larger ones.
...
\begin{enumerate}
    \item All queries executed via official Python drivers:
    \begin{itemize}
        \item ClickHouse: \texttt{clickhouse-driver==0.2.6}
        \item Elasticsearch: \texttt{elasticsearch==8.11.0}
    \end{itemize}
    \item Timing captured using \texttt{time.time()} immediately before and after query execution
    \item Time includes: network roundtrip + query execution + result transfer
    \item Each query run 5 times with cold starts (no caching between runs)
    \item Results reported as: average, minimum, and maximum times
\end{enumerate}

\textbf{Storage Measurement:}
...
FROM system.parts
WHERE database = 'healthcare_benchmark'
  AND active
GROUP BY table;
\end{lstlisting}
...
stats = es.indices.stats(index='medical_events')
size_bytes = stats['indices']['medical_events']
             ['total']['store']['size_in_bytes']
\end{lstlisting}

Storage was measured after all data loading completed and indexes were fully built.

\textbf{Network Latency Baseline:}

To understand network overhead, we measured baseline roundtrip times:
\begin{itemize}
    \item ClickHouse: \texttt{SELECT 1} executed 100 times, averaged: 80ms
    \item Elasticsearch: Simple search on empty index, 100 times, averaged: 53ms
    \item 27ms difference explains some query time variance on small datasets
\end{itemize}

\textbf{Ingestion Speed:}

Data loading measured using bulk insert APIs:
\begin{itemize}
    \item ClickHouse: \texttt{INSERT INTO table VALUES} with batches of 10,000 rows
    \item Elasticsearch: \texttt{\_bulk} API with batches of 1,000 documents
    \item Timing started before first batch, ended after last batch confirmed
\end{itemize}

\subsection{Reproducibility}

All code, queries, and data generation scripts are available in our GitHub repository. To reproduce:

\begin{enumerate}
    \item Clone repository: \texttt{github.com/yourusername/db-benchmarks}
    \item Set up ClickHouse Cloud and Elastic Cloud accounts (free tiers available)
    \item Configure \texttt{config.env} with connection credentials
    \item Generate data: \texttt{python data/generate\_datasets.py}
    \item Load data: \texttt{python benchmarks/load\_*.py}
    \item Run benchmarks: \texttt{python benchmarks/run\_benchmarks.py}
\end{enumerate}

Results stored in \texttt{results/*.json} files with timestamps and full execution metadata.
...
We created two datasets designed to test different scales and use cases:

\textbf{Synthetic Healthcare Data (160,000 rows):}
\begin{itemize}
    \item \textit{Patients table}: 10,000 records with patient\_id (UUID), age (18-95), gender, medical\_condition, registration\_date, and risk\_score
    \item \textit{Medical Events table}: 100,000 records with event\_id, patient\_id (foreign key), event\_type, department, timestamp, duration\_minutes, severity (Low/Medium/High/Critical), cost\_usd
    \item \textit{IoT Telemetry table}: 50,000 records with reading\_id, device\_id, patient\_id, device\_type, timestamp, value, unit, is\_abnormal
    \item \textit{Generation method}: Python's Faker library with realistic distributions. Cost follows normal distribution (\$500-\$5000). Timestamps span 2024-01-01 to 2024-12-31.
\end{itemize}

\textbf{Real-World NYC Taxi Data (13 Million rows):}
\begin{itemize}
    \item \textit{Source}: NYC Taxi \& Limousine Commission official trip records
    \item \textit{Coverage}: January-April 2024 (4 months)
    \item \textit{Format}: Downloaded as Parquet files (~1.7 GB compressed)
    \item \textit{Fields}: tpep\_pickup\_datetime, tpep\_dropoff\_datetime, passenger\_count, trip\_distance, fare\_amount, total\_amount, payment\_type, PULocationID, DOLocationID
    \item \textit{Actual loaded}: ClickHouse (13.07M rows in 184 sec), Elasticsearch (13.07M rows in 1,636 sec)
\end{itemize}

\subsection{The Benchmarks}

We ran seven different query types, each one testing a different capability. Each benchmark was executed 5 times, and we report the average execution time. Query times were measured using Python's \texttt{time.time()} with microsecond precision.

\begin{enumerate}
    \item \textbf{Simple Aggregation:} COUNT and AVG grouped by single dimension
    \begin{lstlisting}[language=SQL,basicstyle=\small\ttfamily]
SELECT department, COUNT(*), AVG(cost_usd)
FROM medical_events
GROUP BY department
    \end{lstlisting}
    
    \item \textbf{Multi-Level GROUP BY:} Multi-dimensional grouping
    \begin{lstlisting}[language=SQL,basicstyle=\small\ttfamily]
SELECT department, severity,
       COUNT(*), AVG(duration_minutes)
FROM medical_events
GROUP BY department, severity
    \end{lstlisting}
    
    \item \textbf{Time-Series Aggregation:} Daily aggregations over time
    \begin{lstlisting}[language=SQL,basicstyle=\small\ttfamily]
...
GROUP BY toDate(timestamp)
    ...
    \item \textbf{Filter + Aggregation:} Selective filtering before aggregation
    \begin{lstlisting}[language=SQL,basicstyle=\small\ttfamily]
SELECT department, COUNT(*), AVG(cost_usd)
FROM medical_events
...
GROUP BY department HAVING COUNT(*) > 100
    ...
    \item \textbf{JOIN Performance:} Cross-table analytical queries
    \begin{lstlisting}[language=SQL,basicstyle=\small\ttfamily]
SELECT p.patient_id, p.age,
       COUNT(e.event_id), AVG(e.cost_usd)
FROM patients p
JOIN medical_events e
  ON p.patient_id = e.patient_id
GROUP BY p.patient_id, p.age
HAVING COUNT(e.event_id) > 5
    \end{lstlisting}
    
    \textit{Note}: Elasticsearch doesn't support SQL JOINs. We implemented this as two separate queries (fetch patients, then fetch events for those patients) and merged results in Python, representing how Elasticsearch users typically handle this.
    
    \item \textbf{Complex Analytical Query:} Subqueries and advanced SQL
    \begin{lstlisting}[language=SQL,basicstyle=\small\ttfamily]
SELECT department, COUNT(*), AVG(cost_usd)
FROM medical_events
WHERE cost_usd > (
...
GROUP BY department HAVING COUNT(*) > 10
    ...
    We executed 5 different aggregation queries in parallel using Python's \texttt{threading} module and measured total elapsed time until all queries completed.
\end{enumerate}
...
ClickHouse & 2.09 MiB \\
Elasticsearch & 31.35 MB \\ 
\textbf{Difference} & \textbf{15.0x} \\ \bottomrule
\end{tabular}
\end{table}

ClickHouse used 15.0 times less storage for the exact same data. This wasn't a fluke. Every single table showed similar compression:
...
\textbf{NYC Taxi Dataset Storage (13.07M rows):}

The compression advantage continued at scale with the NYC taxi dataset:

\begin{itemize}
    \item ClickHouse: 206.61 MiB (0.202 GB) - 8.4x compression from 1.7 GB Parquet
    \item Elasticsearch: 1.718 GB - slight expansion (1.01x) from original
    \item \textbf{Result: 8.5x storage advantage for ClickHouse}
\end{itemize}

This demonstrates that ClickHouse's compression advantage scales with data size, making it extremely cost-effective for large-scale analytics.

\subsection{Data Loading: ClickHouse is 8.9x Faster}

Loading the complete 13 million NYC taxi records showed a clear winner:
...
We ran benchmarks on both systems processing the complete 13.07M row dataset. Both systems successfully loaded and queried the full dataset, providing a fair head-to-head comparison at scale.

\begin{table}[h]
\centering
\caption{Scalability Test (Both systems on complete 13.07M rows)}
\label{tab:nyc}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Benchmark} & \textbf{ClickHouse} & \textbf{Elasticsearch} \\ \midrule
Simple Aggregation & 88ms & 47ms \\
Multi-Level GROUP BY & 92ms & 61ms \\
...
\begin{itemize}
    \item \textbf{Ingestion Speed:} 8.9x faster data loading (70,871 vs 7,987 rows/sec).
    \item \textbf{Storage:} Healthcare 15x compression, NYC 8.5x compression advantage.
    \item \textbf{JOINs:} 2.3x faster on healthcare data with native SQL support.
\end{itemize}
...
\begin{itemize}
    ...
    \item Storage costs matter (15x healthcare, 8.5x NYC compression adds up fast)
    ...
    \item You need fast data ingestion (8.9x faster loading)
\end{itemize}
...
\begin{enumerate}
    \item \textbf{Storage compression is massive.} ClickHouse's 15x (healthcare) and 8.5x (NYC) advantage means huge cost savings at scale. This is the most significant finding.
    ...
    \item \textbf{Data loading is 8.9x faster.} For pipeline workloads with constant ingestion, ClickHouse wins easily.
...
\begin{itemize}
    \item Lower costs from compression (our 15x healthcare and 8.5x NYC advantages validate their storage savings)
    \item Faster complex queries with native SQL JOINs (our 2.3x speedup matches their experience)
    \item 8.9x faster data ingestion for continuous pipeline workflows
