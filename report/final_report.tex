...
\begin{itemize}
    \item \textbf{ClickHouse Cloud:} Version 25.10.2.65, us-east-1 region, Development tier
    \item \textbf{Elasticsearch Cloud:} Version 7.17.4, us-east-1 region, Standard tier
...
\subsection{Real-World Benchmark: NYC Taxi Data}

To validate our findings on a larger, non-synthetic dataset, we benchmarked the NYC Yellow Taxi dataset (January 2024, approx. 3 million rows).

\subsubsection{Data Loading Performance}
\begin{itemize}
    \item \textbf{ClickHouse:} 35.46 seconds (83,602 rows/sec)
    \item \textbf{Elasticsearch:} 301.92 seconds (9,819 rows/sec)
\end{itemize}
\textbf{Finding:} ClickHouse ingested data \textbf{8.5x faster} than Elasticsearch. This confirms ClickHouse's superior write performance for bulk analytics data.

\subsubsection{Query Performance (3 Million Rows)}
\begin{table}[h]
\centering
\caption{NYC Taxi Benchmark Results (milliseconds)}
\label{tab:nyc_performance}
\begin{tabular}{@{}lll@{}}
\toprule
\textbf{Benchmark} & \textbf{ClickHouse} & \textbf{Elasticsearch} \\ \midrule
Simple Aggregation & 110ms & 62ms \\
Multi-Level GROUP BY & 108ms & 51ms \\
\textbf{Time-Series} & \textbf{105ms} & \textbf{127ms} \\
Filter + Aggregation & 113ms & 49ms \\
JOIN Performance & 121ms & 93ms \\
Complex Analytical & 143ms & 68ms \\
Concurrent Load & 239ms & 174ms \\ \bottomrule
\end{tabular}
\end{table}

\textbf{Key Finding:} On the larger NYC taxi dataset (3M rows), Elasticsearch maintained faster performance on 6 of 7 benchmarks (1.3-2.3x faster). However, ClickHouse won the Time-Series Aggregation benchmark (1.2x faster), demonstrating its strength in date-based analytical queries.

\textbf{Analysis:} The NYC taxi results show similar patterns to the synthetic data, with Elasticsearch performing better on simple aggregations due to network latency advantages. ClickHouse's win on Time-Series queries highlights its optimization for date-partitioned analytical workloads, which is particularly relevant for time-series data like taxi trips.

\subsection{Query Performance Summary}

We implemented the same seven benchmark tests on both synthetic healthcare data and real-world NYC taxi data.

\begin{table}[h]
\centering
\caption{Synthetic Data Performance Comparison (milliseconds)}
...
Concurrent Load (5 queries) & 165ms & 115ms \\ \bottomrule
\end{tabular}
\end{table}

\textbf{Key Finding:} Elasticsearch demonstrated faster query performance on 6 of 7 benchmarks for this small dataset (1.4-2.1x faster). However, ClickHouse showed a significant advantage (2.3x faster) on JOIN operations.

\subsubsection{Performance Analysis by Benchmark Type}

\textbf{Aggregation Queries (Benchmarks 1-4, 6):} Elasticsearch consistently outperformed ClickHouse, with query times 44-55ms vs 80-93ms (1.4-2.1x faster).

\textbf{JOIN Operations (Benchmark 5):} ClickHouse demonstrated native SQL JOIN support with 88ms execution time. Elasticsearch required application-side JOIN simulation (two separate queries), resulting in 207ms execution time. \textbf{ClickHouse was 2.3x faster}, highlighting a key architectural advantage.

\textbf{Concurrent Workload (Benchmark 7):} Testing with 5 simultaneous queries showed Elasticsearch handling concurrent load better (115ms vs 165ms).
